...
### Lesson 6: Web3.js Frontend

- Result: received level 3.0 The Phantom of Web3 zombie
- Share: [The Phantom of Web3 Zombie Army](https://share.cryptozombies.io/ko/lesson/6/share/The_Phantom_of_Web3?id=Y3p8NjczNTA4)
- Learned:
  - Connecting a frontend to a deployed smart contract with ABI and address
  - Initializing Web3.js through a wallet provider
  - Tracking the active user account from MetaMask
  - Reading contract state with `.call()`
  - Sending transactions with `.send({ from })`
  - Sending Ether values with `web3.utils.toWei`
  - Listening to `Transfer` events and refreshing UI state
  - Rendering a user's zombie army from `getZombiesByOwner`
...
- Lesson 6 source: `references/cryptozombies-lesson-code/lesson-6`
...
- GitHub Lesson 6 source: [lesson-6](https://github.com/CryptozombiesHQ/cryptozombies-lesson-code/tree/master/lesson-6)
...
CryptoZombies Lesson 6에서는 Web3.js 프론트엔드에서 ABI와 컨트랙트 주소를 사용해 컨트랙트 인스턴스를 만들고, 지갑 계정으로 읽기 호출과 트랜잭션을 실행하는 흐름을 실습했습니다.

`call`과 `send`의 차이, MetaMask 계정 추적, 이벤트 구독, 트랜잭션 상태 메시지 업데이트를 통해 DApp UI가 온체인 상태와 동기화되는 기본 구조를 확인했습니다.

## Next Steps

- Study smart contract deployment and keep adding progress notes.
